...
一个可以直接在终端玩的 **1 名人类 + 4/5 个 LLM agent** 社交推理游戏。保留 Avalon 基础规则，AI 依次给出自然语言发言、简短判断依据和一张 social card。善良 AI 维护独立判断与画像；邪恶 AI 共享代码维护的战略状态，并各自生成自然发言。
...
5. 收齐所有发言后，善良 AI 按最近模型策略投票，邪恶 AI 按代码策略投票和协调任务牌。提案被否决后，新队长重新选队，所有人重新发言。

善良 AI 的模型计划包含身份和行为估计、策略、选队排名、投票阈值和社交行动。邪恶 AI 的模型只返回社交行动，不能设置战略、概率、队伍、投票、任务牌或刺杀目标。投票、任务执行和刺杀本身不额外调用 LLM。

没有失败时，每个 AI 每次发言调用一次，善良 AI 队长选队另加一次；邪恶队长选队不额外调用模型。普通五人局每次提案共 4–5 次请求，六人局 5–6 次；全 AI 演示分别为 5–6、6–7 次。重试会增加实际请求数。成功计划仅在同一「任务轮 / 提案 / 阶段」内缓存，避免重复请求，不跨提案复用发言。结束时 `[CALLS]` 列出每位 agent 每个任务轮的实际 HTTP 尝试数，包含失败和重试。
...
每次只分配一个主目标和最多一个次目标。卡牌范围、目标和结构化行动由代码决定，具体攻击、辩护、施压和解释由 LLM 生成。两名邪恶 AI 不共享完整计划或内部推理。公开文字泄露明显身份、战术字段或目标名称时，会重新请求；耗尽重试即停止，不生成固定替代台词。公开错误统一显示 `invalid_plan`，内部原因 `private_disclosure` 仅出现在开发者调试通道，避免由错误类型暴露阵营。
