"""Localhost-only JSON bridge for the Godot frontend."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/state":
            with STORE.lock:
                self._json(200, {"ok": True, "phase": "START"} if STORE.session is None else STORE.session.state())
            return
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Local bridge for the Avalon Godot UI.")
