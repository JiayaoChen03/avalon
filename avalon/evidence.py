...
from .mission_likelihood import MissionLikelihoodConfig, MissionOutcomeLikelihood, public_outcome
...
class LikelihoodConfig:
    ...
    mission: MissionLikelihoodConfig = field(default_factory=MissionLikelihoodConfig)
    ...
    def __post_init__(self):
        if isinstance(self.mission, dict):
            self.mission = MissionLikelihoodConfig(**self.mission)
        if not isinstance(self.mission, MissionLikelihoodConfig) or any(type(v) is not bool for v in
                (self.behavioral_enabled, self.social_enabled, self.language_enabled)):
            raise ValueError("Invalid likelihood switches")
...
class EvidenceExtractor:
    def __init__(self, observer_id, player_ids, rules, config=None):
    ...
    def extract(self, observation):
        ...
        if o.type == "MISSION_RESULT":
            ...
            outcome = public_outcome(payload, rules)
            constraints = (mission_result_constraints(payload["team"], payload["fail_count"], rules,
                                                     success=outcome["success"])
                           if payload.get("fail_count") is not None else {})
            result = [Evidence(**{**base, "target_agents": list(payload["team"])}, evidence_type=EvidenceType.HARD,
                             feature="mission_result", metadata={"signal_id": f"mission:{o.round}:{o.proposal}",
                                **outcome, **constraints})]
            model = MissionOutcomeLikelihood(self.config.mission)
            if model.skip_reason(outcome) is None:
                result.append(Evidence(**{**base, "target_agents": list(payload["team"])},
                    evidence_type=EvidenceType.BEHAVIORAL, feature="mission_outcome",
                    metadata={"signal_id": f"mission_outcome:{o.round}:{o.proposal}", **outcome,
                              "likelihood_version": model.version, "q": model.config.q,
        ...
        return [Evidence(**{**base, "target_agents": targets}, evidence_type=EvidenceType.BEHAVIORAL,
                         ...
                         "social_action": payload.get("kind") in {"SOCIAL", "REACT", "CHALLENGE_RESPONSE"}})]
...
class LikelihoodModel:
    ...
    def allows(self, hypothesis, evidence):
        ...
        if evidence.feature == "mission_result":
            ...
            if "outcome_observation_type" in data:
                return MissionOutcomeLikelihood.feasible(evil_count, data)
    ...
    def evaluate(self, hypothesis, evidence):
        ...
        if evidence.feature == "mission_outcome":
            count = sum(self.alignments[hypothesis.roles[p]] == "EVIL" for p in evidence.metadata["team"])
            return MissionOutcomeLikelihood(self.config.mission).evaluate(count, evidence.metadata)
        if evidence.evidence_type == EvidenceType.BEHAVIORAL and (not self.config.behavioral_enabled
                or not self.config.social_enabled and evidence.metadata.get("social_action")):
            return 1.0
        if evidence.evidence_type == EvidenceType.LANGUAGE and not self.config.language_enabled:
            return 1.0
