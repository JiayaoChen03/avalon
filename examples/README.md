...
```sh
python -m avalon --players 5 --seed 12 --log examples/deepseek-live-5.jsonl
```
...
- 第一轮前抽中 P3 为首任队长，之后按 P4、P5、P1、P2 轮换。
- 5 个任务轮，5 次选队、25 次社交行动、25 次公开投票，13 个秘密任务票提交回执和 5 次任务结果。
- 共 **20 次真实 API 请求**，每位 agent 每轮恰好一次；20 个计划全部通过校验，**0 次 fallback**。刺杀没有增加调用。
- 善良三次任务成功，刺客选择 P3，Merlin 存活，最终 GOOD 获胜；退出码 0。
- 本局实际出现 HEDGE、PRESSURE；五种牌均由下方 mock 对局和回归测试覆盖。
...
运行后解析公开日志，检查首任抽签先于第一轮；逐个提案检查 **所有玩家各有一条社交行动和一条投票**；逐个任务检查队员提交回执齐全；确认结束后才出现 REVEAL，公开日志不含 belief/profile 或个人任务牌。mock 计数保存在 [smoke-summary.json](smoke-summary.json)，真实 API 计数保存在 [deepseek-live-summary.json](deepseek-live-summary.json)。终端记录中的 `[PRIVATE]` 仅为 P1 自己合法可见的信息，JSONL 不含这些提示。

## 本地 API 与回归测试

`python -m unittest discover -s tests -v` 共 **32 项测试通过**，其中包含：
